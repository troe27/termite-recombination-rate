...
def get_rho_gene_flank(data, row, flanksize, buffer):
    ...
    ss_uflank = data.loc[data.Scaffold==row.scaffold2].loc[(data.Pos_kb_start*1e3)<(row.start-buffer)].loc[(data.Pos_kb_end*1e3)>row.start-(flanksize+buffer)]
    rho_uflank = get_weighted_mean_rho(ss_data=ss_uflank, start=flank_start, stop=(row.start-buffer))

    ss_dflank = data.loc[data.Scaffold==row.scaffold2].loc[(data.Pos_kb_start*1e3)<row.stop+flanksize+buffer].loc[(data.Pos_kb_end*1e3)>row.stop+buffer]
