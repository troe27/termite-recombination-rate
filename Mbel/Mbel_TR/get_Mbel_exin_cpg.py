...
def run():
    ...
    for i, k in gene_pos.iterrows():
        cpg = cpg_for_gene_and_flanking_regions(start=k.Start_pos, stop=k.End_pos, scaffold=s2[k.scaffold], skipflank=True)
        results.append([k.gene_id]+cpg)
    rdf = pd.DataFrame(results)
    rdf.columns = ['ID', 'gene_cpg_e','gene_cpg_o', 'gene_cpg_oe']
    outfile = args.input.rsplit('.',1)[0]+'.ex_in.cpg'.format(flanksize=args.flanksize)
