...
def cpg_for_gene_and_flanking_regions(start, stop, scaffold, flanksize=1e3,buffer=50e3, skipflank=False, strand='+'):
    if strand =='+':
...
#        print(k)
        cpg = cpg_for_gene_and_flanking_regions(start=k.start, stop=k.stop, scaffold=s2[k.scaffold2], flanksize=int(args.flanksize), buffer=int(args.flankbuffer))
        results.append([k.idstring]+cpg)
    ...
    outfile = args.input.rsplit('.',1)[0]+'.gene_{flanksize}flank_{buffer}buffer.cpg'.format(flanksize=str(args.flanksize), buffer=str(args.flankbuffer))
